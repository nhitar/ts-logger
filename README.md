...
`Express` приложение с `REST API`, `JWT` + `bcrypt` авторизацией, CRUD-эндпоинтами, интеграцией с `sqlite3` и `Jest` тестами.

Приложение представляет из себя `REST API` для управления валютами с историей цен и кошельками, а также фоновой задачей по синхронизации данных из публичного `Binance API`.

Реализованные эндпоинты:

- `CRUD` управление списком валют и кошельков;

- Получение истории цен валют и высоты блокчейна;

- Получение цен валют по тикеру;

- Покупка/продажа валюты с привязкой к кошельку.

Основные эндпоинты защищены через `JWT` + `bcrypt`, получить доступ можно через авторизацию с отладочными данными.

Все эндпоинты покрыты `Jest` тестами с настроенным `CI`. Сами эндпоинты описаны в `OpenAPI` файле, на основе которого работает `Swagger UI`.
